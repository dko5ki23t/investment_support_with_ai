...
logger = Logger(__name__, 'model_lstm_4.log')
...
out_estimate_directory_default = os.path.join(os.path.dirname(__file__), '../../db/estimates/estimate_lstm_4')
model_file_default = os.path.join(os.path.dirname(__file__), '../../model/model_lstm_4')
...
def description():
    ...
    説明
    """
    return 'LSTMによる推定を行う。一定期間(window_size日数分)の終値と出来高をもとに、次の日の（高値 - 始値）を推定する。与えられたデータの後半20%を用いて推定値を出し、評価値も出す。ただし、学習に使用するデータは最長約5年分とする。'
...
def estimate(stock_df: pl.DataFrame, out_file: str, model_file='', window_size=128, show_figure=False, *discard):
    ...
    sliced_stock_df = stock_df[-245 * 5:]
    code = sliced_stock_df.get_column('Code')[0]
    closes = sliced_stock_df['Close'].to_numpy().reshape(-1, 1)
    volumes = sliced_stock_df['Volume'].to_numpy().reshape(-1, 1)
    # 高値-始値の差分
    stock_df_with_delta = sliced_stock_df.with_columns((pl.col('High') - pl.col('Open')).alias('Delta'))
    ...
    # 予想を出力        
    out_list = []
    prev_close = sliced_stock_df.filter(pl.col('Date') == predict.get_column('Date')[0]).get_column('Close')[0]
    # TODO: iter_rows()は非推奨
    i = 0
    for row in predict.iter_rows():
        ...
        tmp = sliced_stock_df.filter(pl.col('Date') == row[0])
        if len(tmp) > 0:
    ...
    output = {
        ...
        "method_name": name(),
        "version": version(),
        "last_date": sliced_stock_df.get_column('Date')[-1],
        "estimate": out_list,
    }
    # ファイル出力
