...
class model_3:
    ...
    def first_compile(self, stock_df: pl.DataFrame, out_file: str, show_figure = False):
        # 高値-始値の差分
        ...
        test_data_y = scaled_deltas[training_data_len - window_size:, :]
        #test_data = scaled_Y
        ...
        for row in predict.iter_rows():
            if row[1] > 0:
                # TODO: 出力するスコアは未定
                out_list.append({"date": row[0], "code": self.code, "gains": row[1], "score": 0})
        output = {"estimate": out_list}
        # ファイル出力
...
def estimate_gen(input: str, output: str, filter_market_code = 0):
    input_internal = input
    if input_internal == '':
        input_internal = input_directory_default
    # 銘柄情報ファイル読み込み
    stock_info_file = stock_info_file_default
    try:
        stock_info_df = pl.read_csv(stock_info_file)
    except:
    ...
    for index in tqdm.tqdm(range(len(data_files))):
        ...
        model.first_compile(stock_df, out_strategy_file)
        yield [index, len(data_files)]
...
def main():
    args = set_argparse()
    # 銘柄情報ファイル読み込み
    stock_info_file = args.stock_info
    if stock_info_file == '':
        stock_info_file = stock_info_file_default
    try:
        stock_info_df = pl.read_csv(stock_info_file)
    ...
    for index in tqdm.tqdm(range(len(data_files))):
        ...
        # 市場コードでフィルタリング
        market_code = stock_info_df.filter(pl.col('Code') == code).get_column('MarketCode')[0]
        if args.filter_market_code != 0 and market_code != args.filter_market_code:
            continue
        # 出力先ファイル決定
