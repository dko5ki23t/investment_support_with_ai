...
logger = Logger(__name__, 'model_lstm_6.log')
...
out_estimate_directory_default = os.path.join(os.path.dirname(__file__), '../../db/estimates/estimate_lstm_6')
model_file_default = os.path.join(os.path.dirname(__file__), '../../model/model_lstm_6')
...
def description():
    ...
    説明
    """
    return 'LSTMによる推定を行う。一定期間(window_size日数分)の日経平均・始値・終値・低値・高値・出来高をもとに、次の日の（高値 - 始値）を推定する。与えられたデータの後半20%を用いて推定値を出し、評価値も出す。'
...
def estimate(n225_df: pl.DataFrame, stock_df: pl.DataFrame, out_file: str, model_file='', window_size=128, show_figure=False, *discard):
    """
    LSTMによる推定を行う。
    一定期間(window_size日数分)の日経平均・始値・終値・低値・高値・出来高をもとに、次の日の（高値 - 始値）を推定する。
    ...
    scaled_values = np.stack([
        ...
        scaler7.fit_transform(n225_closes).flatten()], axis=1)
    ...
    x_predict = np.reshape(x_predict, (x_predict.shape[0], x_predict.shape[1], 6))
