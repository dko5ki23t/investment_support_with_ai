...
out_order_directory_default = os.path.join(os.path.dirname(__file__), '../../db/orders/order_2')
...
def description():
    ...
    説明
    """
    return '各日予想に対するスコアが最大の銘柄1種を始値で買って予想利益分の差が出たら売る戦略で注文作成'

def strategy_gen(input: str, output='', base=1000000, stock_info_file='', filter_market_code=0, method_name=''):
    """
    【ジェネレータ】各日予想に対するスコアが最大の銘柄1種を始値で買って予想利益分の差が出たら売る戦略で注文作成
    ...
    for index in tqdm(range(len(estimates))):
        ...
        if math.floor(estimate['gains']) and estimate['yest_close'] * 100 <= base > 0:
