...
out_order_directory_default = os.path.join(os.path.dirname(__file__), '../../db/orders/order_6')
...
def description():
    ...
    説明
    """
    return '各日利益が最大（ただし前日終値単位円当たり）の銘柄1種を始値で買って予想利益分の8割の差が出たら売る戦略で注文作成'

def strategy_gen(input: str, output='', base=1000000, stock_info_file='', filter_market_code=0, method_name=''):
    """
    【ジェネレータ】各日利益が最大（ただし前日終値単位円当たり）の銘柄1種を始値で買って予想利益分の8割の差が出たら売る戦略で注文作成
    ...
    for index in tqdm(range(len(estimates))):
        ...
        if math.floor(estimate['gains'] * 0.8) > 0 and estimate['yest_close'] * 100 <= base:
            if date not in date_to_estimate or date_to_estimate[date]['gains'] / date_to_estimate[date]['yest_close'] < gains_unit:
                # 同じ日の注文なら前日終値単位円あたりの利益が大きい方のみ残す
    ...
    for index in tqdm(range(len(estimates))):
        ...
        # 5営業日経っても売れなかった場合はgainsの0.5倍で売る
        order = {
            "date": estimate['date'],
