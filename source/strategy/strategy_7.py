...
logger = Logger(__name__, 'strategy_7.log')

stock_info_file_default = os.path.join(os.path.dirname(__file__), '../../db/stock_info.csv')
out_order_directory_default = os.path.join(os.path.dirname(__file__), '../../db/orders/order_7')
...
def description():
    ...
    説明
    """
    return '各日の利益（ただし前日終値単位円当たり）×正規化されたスコアが最大の銘柄1種を始値で買って予想利益分の6割の差が出たら売る戦略で注文作成'

def strategy_gen(input: str, output='', base=1000000, stock_info_file='', filter_market_code=0, method_name=''):
    """
    【ジェネレータ】各日の利益（ただし前日終値単位円当たり）×正規化されたスコアが最大の銘柄1種を始値で買って予想利益分の6割の差が出たら売る戦略で注文作成
    ...
    for index in tqdm(range(len(estimates))):
        ...
        date = estimate['date']
        gains_unit = estimate['gains'] / estimate['yest_close']
        if math.floor(estimate['gains'] * 0.6) > 0 and estimate['yest_close'] * 100 <= base:
            scaled_score = 1
            if 'score2' in estimate:
                # score2は0~-1で、0に近いほど良いものとする
                scaled_score = 1 + estimate['score2']
            val = scaled_score * gains_unit
            if date not in date_to_estimate or date_to_val[date] < val:
                if date not in date_to_estimate:
                    logger.info(f'{date} code: {estimate["code"]}, val: {scaled_score} * {gains_unit} = {val}')
                else:
                    logger.info(f'{date} code: {estimate["code"]}, val: {scaled_score} * {gains_unit} = {val} > {date_to_val[date]}')
                # 同じ日の注文なら前日終値単位円あたりの利益×スコアが大きい方のみ残す
                date_to_estimate[date] = estimate
                date_to_val[date] = val
    print('完了')
    print('注文作成・・・')
    # 注文作成
    ...
    for index in tqdm(range(len(estimates))):
        ...
        orders.append(order)
        # 買値との差がgainsの6割を超えたら売る
        # その日に売れなかった場合はgainsの0.3倍で売る
        order = {
            "date": estimate['date'],
            "due": next_business_day(estimate['date']),
            "due2": "max",
            "code": estimate['code'],
            "type": "sell-delta",
            "value": math.floor(estimate['gains'] * 0.6),
            "value2": math.floor(estimate['gains'] * 0.3),
