...
def estimate(data: pd.DataFrame, str_x, str_y):
    ...
    list2 = [data['code'].iloc[0], data['stock name'].iloc[0], model2.name(), result2[0], data[str_x].iloc[-1], result1[1]]
    return (pd.DataFrame([list1, list2], columns=['code', 'stock name', 'model name', 'pipeline', 'last_day', 'MSR']))
