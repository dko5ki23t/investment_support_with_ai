...
def estimate(data: pd.DataFrame, nikkei: pd.DataFrame):
...
#        code, name, days, data.filter(['close']).values, last_timestamp
#    )
    model4 = model_4.model_4(
        code, name, days, data_nikkei.values, last_timestamp
    )
    #return [meta_data, model1, model2, model3, model4]
    return [model4,]
